...
app.get('/', (req, res) => {
    ...
    res.send(returnHTML(data, homeBundle, HomeRoot, "Best Personal Injury Attorney California | Free Legal Consultation Visalia – NRC Law Firms | Nelson Rozier & Christenson", "Do not suffer from other people’s mistakes. Partner with NRC Law, the best personal injury attorneys in California. We represent plaintiffs and defendants.", "Personal injury attorney California, Free Legal Consultation Visalia, Best Injury Attorney, Best Visalia law firm, Visalia top Experienced law firm, Best Legal Attorney Services California, Best Legal attorney California, Experienced Legal attorney Visalia, Visalia Personal Injury Attorneys, experienced attorney California"));
});
...
app.get('/blogcattemplate', (req, res) => {
  let data = "";
  res.set('Cache-Control', 'public, max-age=31557600');
  res.send(returnHTML(data, blogcattemplateBundle, BlogcattemplateRoot, "Blog & News About Personal Injury Attorneys", "Attorneys", ""));
});
app.get('/blog/search/:query', (req, res) => {
  ...
  res.set('Cache-Control', 'public, max-age=31557600');
  res.send(returnHTML(data, blogBundle, BlogRoot, "Blog & News About Personal Injury Attorneys", "Lawyers", ""));
});
app.get('/blog/category/:category', (req, res) => {
  ...
  res.set('Cache-Control', 'public, max-age=31557600');
  res.send(returnHTML(data, blogBundle, BlogRoot, "Blog & News About Personal Injury Attorneys", "Personal Injury", ""));
});
app.get('/blog/:year/:month', (req, res) => {
  ...
  res.set('Cache-Control', 'public, max-age=31557600');
  res.send(returnHTML(data, blogBundle, BlogRoot, "Blog & News About Personal Injury Attorneys", "Personal Injury Attorneys", ""));
});
app.get('/blog/:year/:month/:title', (req, res) => {
  const { year, month, title } = req.params;
  let data = { year, month, title };
  res.set('Cache-Control', 'public, max-age=31557600');
  res.send(returnHTML(data, blogtemplateBundle, BlogtemplateRoot, "Blog & News About Personal Injury Attorneys", "", ""));
});
