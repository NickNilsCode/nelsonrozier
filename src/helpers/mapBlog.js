...
export default function(blogData){
  return blogData.map((a,ai) => {
    ...
    return (
      <BlogDiv key={ai}>
        <a href={`/blog/${year}/${zeromonth}/${titledashed}`}><h2>{a.title}</h2></a>
        <p>
          <a href="/">{author}</a> | {month} {day}, {year} | { intersperse(a.categories.map((b,bi) => <a key={bi} href={`/blog/category/${b.toLowerCase().split(" ").join("-")}`}>{b}</a>), ", ") }
        </p>
